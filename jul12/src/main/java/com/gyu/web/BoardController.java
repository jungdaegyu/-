 ...
 * 
 * 연결 = DI 의존 주입
 * 1. @Autowiered = 스프링에서 제공, 타입이 일치하는 녀석을 연결 
 ...
 *    메서드가 실행되기 전, 실행된 직후, 실행시점에 따라 따로 기능을 적용 시키는 것
 */
@Controller
public class BoardController {
	...
	@Autowired
	private BoardService boardService; //보드서비스랑 연결... 원래는 BoardService boardSerive = new BoardService 이런식으로 했었는데 이렇게 연결
	
	//사용자의 어떤 요청이 들어오면 어떤 일을 하겠습니다.
	@RequestMapping(value = "/main")	
	public String main() { //위 문장과 연결되어서 /main이라는 요청이오면 요게 동작
		return "main"; //jsp파일명 //아래 해당 jsp파일을 열어줍니다. 요청 오면 main.jsp를 열어줘
	}
